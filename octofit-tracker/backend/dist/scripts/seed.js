...
async function seed() {
    ...
    console.log(`Connecting to MongoDB at ${config_1.MONGO_URI}`);
    await mongoose_1.default.connect(config_1.MONGO_URI);
    const existingCollections = [User_1.UserModel, Team_1.TeamModel, Activity_1.ActivityModel, Workout_1.WorkoutModel, LeaderboardEntry_1.LeaderboardEntryModel];
    await Promise.all(existingCollections.map((model) => model.deleteMany({})));
    const users = [
        {
            _id: 'u1',
            name: 'Ari Octo',
    ...
    await Team_1.TeamModel.insertMany(teams);
    await Activity_1.ActivityModel.insertMany(activities);
    await Workout_1.WorkoutModel.insertMany(workouts);
    await LeaderboardEntry_1.LeaderboardEntryModel.insertMany(leaderboardEntries);
